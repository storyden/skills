...
func main() {
	...
	// Register event handlers before running the plugin runtime loop.
	plugin.OnThreadReplyCreated(bot.onThreadReplyCreated)

	// Run connects to Storyden RPC and starts processing incoming events.
...
func (r *reactBot) onThreadReplyCreated(ctx context.Context, event *rpc.EventThreadReplyCreated) error {
	...
	// Build an authenticated API client from the manifest access declaration.
	client, err := r.plugin.BuildAPIClient(timeoutCtx)
	if err != nil {
		return fmt.Errorf("build api client: %w", err)
	}

	// React directly to the newly created reply post.
